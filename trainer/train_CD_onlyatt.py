...
from dataloader.data_loaders import *
...
data_path_20= f"/prepare_datasets/data_edf_20_npz/fpzcz"
data_path_78= f"/prepare_datasets/data_edf_78_npz/fpzcz"
...
#    x_domains = [("a", "b"), ("a", "c"), ("b", "a"), ("b", "c"), ("c", "a"), ("c", "b")]
    ...
    for i in x_domains:
        ...
        for run_id in range(args.num_runs):
            ...
            '''
            src_train_dl, src_valid_dl, src_test_dl,data_count = data_generator_np(SrcFoldData[fold_id][0],
                                                                           SrcFoldData[fold_id][1], SrcFoldData[fold_id][2],128)
            trg_train_dl, trg_valid_dl, trg_test_dl,data_count = data_generator_np(TrgFoldData[fold_id][0],
                                                                           TrgFoldData[fold_id][1], TrgFoldData[fold_id][2],128)
            print("len of src train dl",len(src_train_dl))
            print("len of src valid dl",len(src_valid_dl))
            print("len of src test dl",len(src_test_dl))
            print("len of trg train dl",len(trg_train_dl))
            print("len of trg valid dl",len(trg_valid_dl))
            print("len of trg test dl",len(trg_test_dl))
            print("src id",src_id)
            print("trg id",trg_id)
...
if __name__ == "__main__":
    ...
    TrgFoldData = load_folds_data(data_path_20, 20)
    SrcFoldData = load_folds_data(data_path_78,20)
    main_train_cd()
