...
from trainer.training_evaluation_onlyatt import model_evaluate,val_self_training
...
def cross_domain_train(src_train_dl, trg_train_dl, src_valid_dl,
                       src_id, trg_id,
...
#    feature_extractor = cnn_feature_extractor(model_configs).float().to(device)
    ...
    for round_idx in range(param_config.self_training_iterations):
        ...
        for epoch in range(1, configs.num_epoch + 1):
            ...
            srcdata_loader = enumerate(src_train_dl)
            ...
            for step, (src_data,src_labels) in srcdata_loader:
                src_data, src_labels = src_data.float().to(device),src_labels.long().to(device)
            ...
            if epoch % 1 == 0:
                ...
                #target_loss, target_score, _, _ = model_evaluate(
                #   (feature_extractor1,(classifier_1,classifier_2)),trg_valid_dl,device,trg_data_count)
                src_loss, src_score, _, _ = model_evaluate(
                    (feature_extractor1,(classifier_1,classifier_2)), src_valid_dl,device)
                ...
                logger.debug(f'[Epoch : {epoch}/{configs.num_epoch}]')
                logger.debug(
                    f'{args.da_method} Loss  : {src_loss:.4f}\t | \t{args.da_method} Accuracy  : {src_score:2.4f}')
                logger.debug(f'-------------------------------------')
